"""
EKŞİ SÖZLÜK AI
- Tüm başlıkları ve entry'leri çeker
- SQLite ile saklar
- TF-IDF ile arama
- Gradio 5 web arayüzü
"""

import os
import time
import random
import sqlite3
import threading
import logging
from datetime import datetime

import requests
from bs4 import BeautifulSoup
import numpy as np
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity
import gradio as gr

# ── LOGGING ──────────────────────────────────────────────
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger(__name__)

# ── AYARLAR ──────────────────────────────────────────────
BASE_URL         = "https://eksisozluk.com"
DB_DOSYASI       = os.environ.get("DB_PATH", "eksi.db")
HF_TOKEN         = os.environ.get("HF_TOKEN", "")
PORT             = int(os.environ.get("PORT", 7860))
GECIKME_MIN      = 2.5
GECIKME_MAX      = 5.0
RATE_LIMIT_BEKLE = 90
MAX_BASLIK_SAYFA = 50

# Eksi Sozluk resmi mobil API
EKSI_API = "https://api.eksisozluk.com"
EKSI_API_KEY = "e-u-k-i"  # public key

HEADERS = {
    "User-Agent": "ExsiSozluk/6.5.0 (iPhone; iOS 16.0; Scale/3.00)",
    "Accept": "application/json",
    "Accept-Language": "tr-TR",
    "Authorization": f"Bearer {EKSI_API_KEY}",
}

# Fallback web scraping icin
WEB_HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) "
        "AppleWebKit/605.1.15 (KHTML, like Gecko) "
        "Version/16.0 Mobile/15E148 Safari/604.1"
    ),
    "Accept-Language": "tr-TR,tr;q=0.9",
    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
    "X-Requested-With": "XMLHttpRequest",
}

# ── VERİTABANI ───────────────────────────────────────────

def db_baglan():
    conn = sqlite3.connect(DB_DOSYASI, check_same_thread=False, timeout=30)
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA journal_mode=WAL")
    conn.execute("PRAGMA synchronous=NORMAL")
    return conn


def db_olustur():
    conn = db_baglan()
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS entryles (
            id          TEXT PRIMARY KEY,
            baslik      TEXT NOT NULL,
            baslik_url  TEXT NOT NULL,
            icerik      TEXT NOT NULL,
            yazar       TEXT,
            tarih       TEXT,
            favori      TEXT DEFAULT '0',
            eklendi     TEXT DEFAULT CURRENT_TIMESTAMP
        );
        CREATE INDEX IF NOT EXISTS idx_baslik ON entryles(baslik);

        CREATE TABLE IF NOT EXISTS basliklar (
            url         TEXT PRIMARY KEY,
            baslik      TEXT NOT NULL,
            tamamlandi  INTEGER DEFAULT 0,
            guncellendi TEXT DEFAULT CURRENT_TIMESTAMP
        );
    """)
    conn.commit()
    conn.close()
    log.info("Veritabani hazir: %s", DB_DOSYASI)


def _say(sql):
    try:
        conn = db_baglan()
        r = conn.execute(sql).fetchone()
        conn.close()
        return r[0] if r else 0
    except Exception:
        return 0


def toplam_say():
    return _say("SELECT COUNT(*) FROM entryles")


def baslik_sayisi():
    return _say("SELECT COUNT(*) FROM basliklar")


def tamamlanan_sayisi():
    return _say("SELECT COUNT(*) FROM basliklar WHERE tamamlandi=1")


def entry_kaydet(entryler):
    if not entryler:
        return 0
    try:
        conn = db_baglan()
        c = conn.executemany(
            """INSERT OR IGNORE INTO entryles
               (id, baslik, baslik_url, icerik, yazar, tarih, favori)
               VALUES (:id, :baslik, :baslik_url, :icerik, :yazar, :tarih, :favori)""",
            entryler,
        )
        conn.commit()
        n = c.rowcount
        conn.close()
        return n
    except Exception as e:
        log.error("entry_kaydet hata: %s", e)
        return 0


def baslik_kaydet(basliklar):
    if not basliklar:
        return
    try:
        conn = db_baglan()
        conn.executemany(
            "INSERT OR IGNORE INTO basliklar (url, baslik) VALUES (:url, :baslik)",
            basliklar,
        )
        conn.commit()
        conn.close()
    except Exception as e:
        log.error("baslik_kaydet hata: %s", e)


def baslik_tamamla(url):
    try:
        conn = db_baglan()
        conn.execute(
            "UPDATE basliklar SET tamamlandi=1, guncellendi=CURRENT_TIMESTAMP WHERE url=?",
            (url,),
        )
        conn.commit()
        conn.close()
    except Exception as e:
        log.error("baslik_tamamla hata: %s", e)


def bekleyen_basliklar():
    try:
        conn = db_baglan()
        rows = conn.execute(
            "SELECT url, baslik FROM basliklar WHERE tamamlandi=0 ORDER BY rowid"
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows]
    except Exception:
        return []


# ── GLOBAL DURUM ─────────────────────────────────────────
durum_g = {
    "aktif":     False,
    "su_an":     "Baslatiliyor...",
    "baslangic": None,
    "hata":      0,
}

# ── HTTP ─────────────────────────────────────────────────

def guvenli_get(url, deneme=3, api_mod=False):
    """
    api_mod=True: JSON API endpoint
    api_mod=False: Web scraping (WEB_HEADERS kullan)
    """
    hdrs = HEADERS if api_mod else WEB_HEADERS
    for i in range(deneme):
        try:
            r = requests.get(url, headers=hdrs, timeout=20)
            if r.status_code == 200:
                return r
            if r.status_code == 429:
                bk = RATE_LIMIT_BEKLE * (i + 1)
                log.warning("Rate limit! %ds bekleniyor...", bk)
                time.sleep(bk)
                continue
            if r.status_code in (404, 410):
                return None
            if r.status_code == 403:
                log.warning("403 engel: %s", url[-60:])
                time.sleep(20 * (i + 1))
                continue
            time.sleep(5 * (i + 1))
        except requests.RequestException as e:
            log.warning("Istek hatasi (%d/%d): %s", i + 1, deneme, e)
            time.sleep(10 * (i + 1))
    return None


# ── EKŞİ MOBİL API ───────────────────────────────────────

def api_basliklar_cek(sayfa=1):
    """Resmi Eksi API ile baslik listesi cek."""
    try:
        r = requests.get(
            f"{EKSI_API}/topic/popular?p={sayfa}&take=50",
            headers=HEADERS,
            timeout=15,
        )
        if r.status_code != 200:
            return []
        data = r.json()
        items = data.get("data", {}).get("items", []) or data.get("items", [])
        sonuc = []
        for item in items:
            slug  = item.get("slug") or item.get("name", "")
            title = item.get("title") or item.get("name", "")
            tid   = item.get("id", "")
            if slug:
                url = f"{BASE_URL}/{slug}--{tid}" if tid else f"{BASE_URL}/{slug}"
                sonuc.append({"url": url, "baslik": title or slug})
        return sonuc
    except Exception as e:
        log.error("api_basliklar_cek hata: %s", e)
        return []


def api_entry_cek(baslik_id, sayfa=1):
    """Resmi Eksi API ile entry cek."""
    try:
        r = requests.get(
            f"{EKSI_API}/topic/{baslik_id}?p={sayfa}&take=20",
            headers=HEADERS,
            timeout=15,
        )
        if r.status_code != 200:
            return [], False
        data    = r.json()
        payload = data.get("data", {}) or data
        items   = payload.get("entries", []) or payload.get("items", [])
        baslik_ad = payload.get("title", "")
        entryler  = []
        for e in items:
            eid    = str(e.get("id", ""))
            icerik = e.get("content", "") or e.get("text", "")
            yazar  = e.get("author", {}).get("nick", "") if isinstance(e.get("author"), dict) else str(e.get("author", ""))
            tarih  = str(e.get("created", ""))
            favori = str(e.get("favoriteCount", "0"))
            if eid and len(icerik) > 5:
                entryler.append({
                    "id": eid,
                    "baslik": baslik_ad,
                    "baslik_url": f"{BASE_URL}/{baslik_id}",
                    "icerik": icerik,
                    "yazar": yazar,
                    "tarih": tarih,
                    "favori": favori,
                })
        page_count = payload.get("pageCount", 1)
        sonraki    = sayfa < page_count
        return entryler, sonraki
    except Exception as e:
        log.error("api_entry_cek hata: %s", e)
        return [], False


# ── TARAYICI ─────────────────────────────────────────────

# Gunluk/populer listeler
LISTE_ENDPOINTLER = [
    "/basliklar/bugun",
    "/basliklar/populer",
    "/basliklar/gundem",
    "/basliklar/kanal",
    "/basliklar/spor",
    "/basliklar/siyaset",
    "/basliklar/magazin",
]

# A-Z harf bazli tarama - tum basliklar buradan
HARFLER = list("abcdefghijklmnopqrstuvwxyz") + [
    "tr-0",  # rakam ile baslayan
]


def url_temizle(href):
    """href'den temiz URL uret."""
    if not href:
        return None
    if "/entry/" in href or href.startswith("/?q="):
        return None
    if href.startswith("/"):
        tam = BASE_URL + href
    elif href.startswith("http"):
        tam = href
    else:
        return None
    if "?p=" in tam:
        tam = tam.split("?p=")[0]
    return tam


def sayfa_parse_basliklar(soup):
    """Soup'tan baslik listesi cikar - tum olasi selector'leri dener."""
    sonuc = {}
    # Tum olasiliklari dene
    selector = (
        "ul.topic-list li a, "
        "ul.partial-topic-list li a, "
        "ul.index-section li a, "
        "ul.index li a, "
        ".index-section a, "
        "#index-section a, "
        ".topic-list a, "
        "li.index-item a"
    )
    for item in soup.select(selector):
        href   = item.get("href", "").strip()
        baslik = item.get_text(strip=True)
        if not baslik or len(baslik) < 2:
            continue
        tam = url_temizle(href)
        if tam and tam not in sonuc:
            sonuc[tam] = baslik
    return sonuc


def liste_basliklar_cek():
    """Bugun/populer/gundem listelerinden baslik topla."""
    toplanan = {}
    for ep in LISTE_ENDPOINTLER:
        for sayfa in range(1, MAX_BASLIK_SAYFA + 1):
            r = guvenli_get(f"{BASE_URL}{ep}?p={sayfa}")
            if not r:
                break
            soup  = BeautifulSoup(r.text, "html.parser")
            bulunan = sayfa_parse_basliklar(soup)
            if not bulunan:
                break
            yeni = {k: v for k, v in bulunan.items() if k not in toplanan}
            toplanan.update(yeni)
            if not yeni:
                break
            time.sleep(1.5)
    return toplanan


def harf_basliklar_cek(harf):
    """
    /index/{harf} sayfalarini tarayarak o harfle baslayan
    tum baslik URL'lerini toplar. Bu sekilde milyonlarca
    basliga ulasabiliriz.
    """
    toplanan = {}
    for sayfa in range(1, 9999):  # pratik ust sinir
        r = guvenli_get(f"{BASE_URL}/index/{harf}?p={sayfa}")
        if not r:
            break
        soup    = BeautifulSoup(r.text, "html.parser")
        bulunan = sayfa_parse_basliklar(soup)
        if not bulunan:
            break
        yeni = {k: v for k, v in bulunan.items() if k not in toplanan}
        toplanan.update(yeni)
        # Son sayfa kontrolu
        if not soup.select_one("div.pager a[rel='next']") and not soup.select_one(".pager .next"):
            break
        if not yeni:
            break
        time.sleep(random.uniform(1.5, 3.0))
    return toplanan


def tum_basliklar_cek():
    """
    1. Once liste sayfalarindan hizli baslik topla
    2. Sonra A-Z index ile tum basliklar
    """
    toplanan = {}

    # Adim 1: liste sayfalari (hizli)
    log.info("Liste sayfalarindan baslik toplanıyor...")
    toplanan.update(liste_basliklar_cek())
    log.info("Liste: %d baslik", len(toplanan))

    # Adim 2: A-Z index (kapsamli)
    log.info("A-Z index taramasi basliyor...")
    for harf in HARFLER:
        durum_g["su_an"] = f"Index tarama: /{harf}"
        yeni = harf_basliklar_cek(harf)
        eski_say = len(toplanan)
        toplanan.update(yeni)
        log.info("/%s: %d yeni baslik (toplam %d)",
                 harf, len(toplanan) - eski_say, len(toplanan))
        time.sleep(random.uniform(2.0, 4.0))

    sonuc = [{"url": u, "baslik": b} for u, b in toplanan.items()]
    log.info("TOPLAM %d baslik bulundu", len(sonuc))
    return sonuc


def entry_cek_sayfa(url, sayfa):
    r = guvenli_get(f"{url}?p={sayfa}")
    if not r:
        log.warning("entry_cek_sayfa: HTTP basarisiz %s", url)
        return [], False

    soup   = BeautifulSoup(r.text, "html.parser")

    # Giris/captcha kontrolu
    if soup.select_one("form#login-form") or "robots" in r.url:
        log.warning("Giris sayfasina yonlendirme: %s", url)
        time.sleep(30)
        return [], False

    bel    = soup.select_one("h1#title a, h1#title")
    baslik = bel.get_text(strip=True) if bel else url.rstrip("/").split("/")[-1]

    # Ham HTML debug (ilk 500 karakter)
    tum_li = soup.select("li[data-id]")
    if not tum_li:
        # Alternatif selector dene
        tum_li = soup.select("li[data-author]")
    log.info("entry_cek_sayfa: %s sayfa=%d baslik=%s li_sayisi=%d",
             url[-40:], sayfa, baslik[:20], len(tum_li))

    entryler = []
    for el in tum_li:
        eid    = el.get("data-id", "").strip()
        if not eid:
            continue
        yazar  = el.get("data-author", "").strip()
        # Icerik icin birden fazla selector dene
        ic = (
            el.select_one("div.content") or
            el.select_one("div[itemprop='text']") or
            el.select_one(".entry-body")
        )
        icerik = ic.get_text(" ", strip=True) if ic else ""
        if len(icerik) < 5:
            continue
        t_el   = el.select_one("footer .entry-date, .entry-date")
        tarih  = t_el.get_text(strip=True) if t_el else ""
        f_el   = el.select_one(".favorite-count, .rate")
        favori = f_el.get_text(strip=True) if f_el else "0"
        entryler.append({
            "id": eid, "baslik": baslik, "baslik_url": url,
            "icerik": icerik, "yazar": yazar,
            "tarih": tarih, "favori": favori,
        })

    log.info("entry_cek_sayfa: %d entry bulundu", len(entryler))

    sonraki = bool(
        soup.select_one("div.pager a[rel='next']") or
        soup.select_one(".pager .next")
    )
    return entryler, sonraki


def baslik_isle(url, baslik):
    sayfa  = 1
    toplam = 0
    while True:
        durum_g["su_an"] = f"'{baslik[:35]}' sayfa {sayfa}"
        entryler, sonraki = entry_cek_sayfa(url, sayfa)
        if entryler:
            toplam += entry_kaydet(entryler)
        if not sonraki:
            break
        sayfa += 1
        time.sleep(random.uniform(GECIKME_MIN, GECIKME_MAX))
    return toplam


def isle_baslik(url, baslik):
    """Bir basligi isle: entry cek, kaydet, tamamla."""
    try:
        n = baslik_isle(url, baslik)
        baslik_tamamla(url)
        if n > 0:
            log.info("'%s' -> %d entry", baslik[:40], n)
        return n
    except Exception as e:
        log.error("Hata '%s': %s", baslik[:40], e)
        durum_g["hata"] += 1
        time.sleep(15)
        return 0


def baglanti_test():
    """Once API, sonra web ile baglanti test et."""
    # 1. Resmi API dene
    try:
        r = requests.get(f"{EKSI_API}/topic/popular?p=1&take=5",
                         headers=HEADERS, timeout=10)
        log.info("API baglanti testi: HTTP %d", r.status_code)
        if r.status_code == 200:
            log.info("Eksi API erisimi OK")
            return "api"
    except Exception as e:
        log.warning("API test hatasi: %s", e)

    # 2. Web scraping dene
    try:
        r = requests.get(BASE_URL, headers=WEB_HEADERS, timeout=10)
        log.info("Web baglanti testi: HTTP %d", r.status_code)
        if r.status_code == 200:
            log.info("Eksi web erisimi OK")
            return "web"
    except Exception as e:
        log.warning("Web test hatasi: %s", e)

    log.error("Eksi Sozluk'e erisim yok!")
    return None


def tarama_thread():
    durum_g["aktif"]     = True
    durum_g["baslangic"] = datetime.now()
    tur = 0
    log.info("Tarama thread baslatildi")
    # Once baglanti test et
    if not baglanti_test():
        log.error("Eksi Sozluk'e erisim yok! 60s sonra tekrar denenecek.")
        time.sleep(60)

    while True:
        tur += 1
        log.info("Tur %d basliyor...", tur)

        # --- 1. Once bekleyenleri isle (varsa) ---
        bekleyenler = bekleyen_basliklar()
        if bekleyenler:
            log.info("%d bekleyen baslik isleniyor...", len(bekleyenler))
            for b in bekleyenler:
                isle_baslik(b["url"], b["baslik"])
                time.sleep(random.uniform(GECIKME_MIN, GECIKME_MAX))

        # --- 2. Liste sayfalarindan baslik topla ve hemen isle ---
        durum_g["su_an"] = f"Tur {tur}: liste sayfalarindan baslik toplaniyor..."
        liste = liste_basliklar_cek()
        yeni_liste = [{"url": u, "baslik": b} for u, b in liste.items()]
        baslik_kaydet(yeni_liste)
        bekleyenler = bekleyen_basliklar()
        log.info("%d bekleyen baslik (liste)", len(bekleyenler))
        for b in bekleyenler:
            isle_baslik(b["url"], b["baslik"])
            time.sleep(random.uniform(GECIKME_MIN, GECIKME_MAX))

        # --- 3. A-Z index: her harfi tara, bulunan basliği hemen isle ---
        log.info("A-Z index taramasi basliyor...")
        for harf in HARFLER:
            durum_g["su_an"] = f"Index: /{harf} harfi taranıyor"
            yeni = harf_basliklar_cek(harf)
            yeni_list = [{"url": u, "baslik": b} for u, b in yeni.items()]
            baslik_kaydet(yeni_list)
            log.info("/%s: %d baslik bulundu", harf, len(yeni_list))
            # Hemen isle
            bekleyenler = bekleyen_basliklar()
            for b in bekleyenler:
                isle_baslik(b["url"], b["baslik"])
                time.sleep(random.uniform(GECIKME_MIN, GECIKME_MAX))

        durum_g["su_an"] = f"Tur {tur} tamamlandi, yeniden basliyor..."
        log.info("Tur %d tamamlandi. Toplam entry: %d", tur, toplam_say())
        time.sleep(300)


# ── TF-IDF ───────────────────────────────────────────────
_vec   = None
_mat   = None
_idler = []
_vlock = threading.Lock()


def tfidf_thread():
    global _vec, _mat, _idler
    while True:
        try:
            conn = db_baglan()
            rows = conn.execute(
                "SELECT id, baslik, icerik FROM entryles ORDER BY rowid DESC LIMIT 50000"
            ).fetchall()
            conn.close()
            if len(rows) < 3:
                time.sleep(30)
                continue
            metinler = [f"{r['baslik']} {r['icerik'][:400]}" for r in rows]
            idler    = [r["id"] for r in rows]
            v   = TfidfVectorizer(max_features=50000, ngram_range=(1, 2),
                                  sublinear_tf=True, min_df=1)
            mat = v.fit_transform(metinler)
            with _vlock:
                _vec   = v
                _mat   = mat
                _idler = idler
            log.info("TF-IDF: %d entry", len(idler))
        except Exception as e:
            log.error("TF-IDF hata: %s", e)
        time.sleep(180)


def benzer_bul(soru, kac=7):
    with _vlock:
        if _vec is None or _mat is None:
            return []
        try:
            sv      = _vec.transform([soru])
            skorlar = cosine_similarity(sv, _mat).flatten()
            en_iyi  = np.argsort(skorlar)[::-1][:kac * 2]
            sonuc   = []
            conn    = db_baglan()
            for idx in en_iyi:
                if skorlar[idx] < 0.03:
                    break
                if idx >= len(_idler):
                    continue
                r = conn.execute(
                    "SELECT * FROM entryles WHERE id=?", (_idler[idx],)
                ).fetchone()
                if r:
                    sonuc.append(dict(r))
                if len(sonuc) >= kac:
                    break
            conn.close()
            return sonuc
        except Exception as e:
            log.error("benzer_bul: %s", e)
            return []


# ── YANIT ────────────────────────────────────────────────
HF_MODEL = "mistralai/Mistral-7B-Instruct-v0.3"


def hf_yanitla(soru, entryler, gecmis):
    if not HF_TOKEN:
        return lokal_yanitla(soru, entryler)

    baglam = "\n\n".join(
        f"[{i+1}] {e['baslik']}: {e['icerik'][:300]}"
        for i, e in enumerate(entryler[:5])
    ) if entryler else "Ilgili entry bulunamadi."

    gecmis_str = ""
    for g in gecmis[-3:]:
        if isinstance(g, dict):
            rol = g.get("role", "")
            icerik = g.get("content", "")
            if rol == "user":
                gecmis_str += f"Kullanici: {icerik}\n"
            elif rol == "assistant":
                gecmis_str += f"Bot: {icerik}\n"

    prompt = (
        "<s>[INST] Sen Eksi Sozluk tarzinda, samimi ve esprili Turkce asistansin.\n\n"
        f"Ilgili entry'ler:\n{baglam}\n\n"
        f"{gecmis_str}"
        f"Kullanici: {soru} [/INST]"
    )

    try:
        r = requests.post(
            f"https://api-inference.huggingface.co/models/{HF_MODEL}",
            headers={
                "Authorization": f"Bearer {HF_TOKEN}",
                "Content-Type": "application/json",
            },
            json={
                "inputs": prompt,
                "parameters": {
                    "max_new_tokens": 400,
                    "temperature": 0.75,
                    "top_p": 0.9,
                    "return_full_text": False,
                },
            },
            timeout=60,
        )
        if r.status_code == 200:
            data = r.json()
            if isinstance(data, list) and data:
                yanit = data[0].get("generated_text", "").strip()
                if yanit:
                    return yanit
        elif r.status_code == 503:
            return "Model isinuyor, 20 saniye sonra tekrar dene."
    except Exception as e:
        log.error("HF API: %s", e)

    return lokal_yanitla(soru, entryler)


def lokal_yanitla(soru, entryler):
    if not entryler:
        return "Bu konuda henuz entry yok, tarama devam ediyor."
    satirlar = [f"**{entryler[0]['baslik']}** basligindan:\n"]
    for e in entryler[:3]:
        ic = e["icerik"][:400] + ("..." if len(e["icerik"]) > 400 else "")
        satirlar.append(
            f"> {ic}\n"
            f"> *- {e['yazar'] or 'anonim'}, {e['tarih'] or '?'} - {e['favori']} fav*\n"
        )
    return "\n".join(satirlar)


# ── DURUM METNİ ──────────────────────────────────────────

def sayac_metni():
    sure_str = ""
    if durum_g["baslangic"]:
        fark = datetime.now() - durum_g["baslangic"]
        s    = int(fark.total_seconds() // 3600)
        dk   = int((fark.total_seconds() % 3600) // 60)
        sure_str = f"\n\n Sure: {s}s {dk}dk"

    return (
        "### Tarama Durumu\n"
        f"**Entry:** {toplam_say():,}\n\n"
        f"**Baslik:** {baslik_sayisi():,}\n\n"
        f"**Tamamlanan:** {tamamlanan_sayisi():,}\n\n"
        f"**Aranabilir:** {len(_idler):,}\n\n"
        f"**Su an:** {durum_g['su_an']}\n\n"
        f"**Hata:** {durum_g['hata']}\n\n"
        f"**Durum:** {'Aktif' if durum_g['aktif'] else 'Durdu'}"
        f"{sure_str}"
    )


# ── SOHBET ───────────────────────────────────────────────

def sohbet(mesaj, gecmis):
    if not mesaj or not mesaj.strip():
        return "", gecmis or []
    gecmis   = gecmis or []
    entryler = benzer_bul(mesaj)
    yanit    = hf_yanitla(mesaj, entryler, gecmis)
    gecmis   = gecmis + [
        {"role": "user",      "content": mesaj},
        {"role": "assistant", "content": yanit},
    ]
    return "", gecmis


# ── VERİTABANINI BURADA OLUŞTUR (import sırasında) ───────
db_olustur()

# ── GRADIO ───────────────────────────────────────────────

CSS = """
.gradio-container { max-width: 960px !important; margin: auto !important; }
footer { display: none !important; }
"""

with gr.Blocks(
    title="Eksi Sozluk AI",
    theme=gr.themes.Soft(primary_hue="lime"),
    css=CSS,
) as demo:

    gr.Markdown(
        "# Eksi Sozluk AI\n"
        "Eksi Sozluk entry'lerinden ogrenen sohbet botu. "
        "Tarama arka planda sureklI devam eder."
    )

    with gr.Row():
        with gr.Column(scale=3):
            chatbot = gr.Chatbot(
                height=500,
                show_label=False,
                type="messages",
            )
            with gr.Row():
                mesaj_box = gr.Textbox(
                    placeholder="Bir sey sor... (Enter = gonder)",
                    scale=5,
                    show_label=False,
                    container=False,
                )
                gonder_btn = gr.Button("Gonder", variant="primary", scale=1)
            temizle_btn = gr.Button("Temizle", size="sm")

        with gr.Column(scale=1):
            sayac_md   = gr.Markdown(sayac_metni())
            yenile_btn = gr.Button("Yenile", size="sm")
            gr.Markdown(
                "---\n**Nasil calisir?**\n\n"
                "1. Eksi'yi tarar\n"
                "2. Entry'leri saklar\n"
                "3. TF-IDF ile arar\n"
                "4. Yanit uretir\n\n"
                "---\nHF_TOKEN env ile\nMistral-7B aktif olur."
            )

    mesaj_box.submit(
        fn=sohbet,
        inputs=[mesaj_box, chatbot],
        outputs=[mesaj_box, chatbot],
    )
    gonder_btn.click(
        fn=sohbet,
        inputs=[mesaj_box, chatbot],
        outputs=[mesaj_box, chatbot],
    )
    temizle_btn.click(
        fn=lambda: ("", []),
        inputs=None,
        outputs=[mesaj_box, chatbot],
    )
    yenile_btn.click(
        fn=sayac_metni,
        inputs=None,
        outputs=[sayac_md],
    )


# ── BAŞLATMA ─────────────────────────────────────────────

if __name__ == "__main__":
    threading.Thread(target=tarama_thread, daemon=True, name="tarayici").start()
    threading.Thread(target=tfidf_thread,  daemon=True, name="tfidf").start()

    log.info("Sunucu basliyor... port=%d", PORT)
    demo.launch(
        server_name="0.0.0.0",
        server_port=PORT,
        share=False,
        show_error=True,
        max_threads=40,
    )
