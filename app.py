...
""")
    ...
    mesaj.submit(sohbet_et, [mesaj, gecmis_state], [mesaj, gecmis_state]).then(
        lambda h: h, [gecmis_state], [chatbot]
    )
    gonder.click(sohbet_et, [mesaj, gecmis_state], [mesaj, gecmis_state]).then(
        lambda h: h, [gecmis_state], [chatbot]
    )
    temizle.click(lambda: ([], []), outputs=[chatbot, gecmis_state])

    # Her 10 saniyede sayacı güncelle
    demo.load(sayac_guncelle, outputs=[sayac], every=10)
