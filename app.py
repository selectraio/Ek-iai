...
def tarama_thread():
    ...
    sayfa = 1
    while True:  # Sonsuza kadar yeni başlık ara
        basliklar = baslik_listesi_cek(sayfa)
        if not basliklar:
            sayfa = 1  # Başa dön, Ekşi her gün yeni başlık ekliyor
        ...
        for b in basliklar:
            ...
            durum_yaz(
                "tamamlanan_urls",
                ",".join(list(tamamlananlar)[-5000:])  # Son 5000 tut
            )
            time.sleep(random.uniform(GECIKME_MIN, GECIKME_MAX))
...
embed_model = None
bellek_vektorler = []    # RAM'de tutulan vektörler
...
def sohbet_et(mesaj, gecmis):
    ...
    if HF_TOKEN:
        yanit = llm_yanitla(mesaj, entryler, gecmis)
    else:
        yanit = lokal_yanitla(mesaj, entryler)
...
def sohbet_ve_goster(mesaj_input, gecmis):
    if not mesaj_input.strip():
        gorunum = []
        for i in range(0, len(gecmis)-1, 2):
            gorunum.append((gecmis[i]["content"], gecmis[i+1]["content"]))
        return "", gecmis, gorunum
    entryler = benzer_bul(mesaj_input)
    if HF_TOKEN:
        yanit = llm_yanitla(mesaj_input, entryler, gecmis)
    else:
        yanit = lokal_yanitla(mesaj_input, entryler)
    gecmis.append({"role": "user", "content": mesaj_input})
    gecmis.append({"role": "assistant", "content": yanit})
    gorunum = []
    for i in range(0, len(gecmis)-1, 2):
        gorunum.append((gecmis[i]["content"], gecmis[i+1]["content"]))
...
""")
    ...
    with gr.Row():
        with gr.Column(scale=3):
            chatbot = gr.Chatbot(label="Sohbet", height=480, show_label=False)
            with gr.Row():
                mesaj = gr.Textbox(
                    placeholder="Bir şey sor... (Enter)",
...
""")
    ...
    mesaj.submit(sohbet_ve_goster, [mesaj, gecmis_state], [mesaj, gecmis_state, chatbot])
    gonder.click(sohbet_ve_goster, [mesaj, gecmis_state], [mesaj, gecmis_state, chatbot])
    temizle.click(lambda: ("", [], []), outputs=[mesaj, gecmis_state, chatbot])
